...
import com.example.ether.videodemo.utils.Geometry;
...
public class ObjectBuilder {
    ...
    static interface DrawCommend {
        void draw();
    }

    static class GeneratedData {
        final float[] vertexData;
        final List<DrawCommend> drawList;

        public GeneratedData(float[] vertexData, List<DrawCommend> drawList) {
    ...
    private final List<DrawCommend> drawList = new ArrayList<>();
     ...
     * @return 侧面顶点的数量
     */
    private static int sizeOfOpenCylinderInVertex(int numPoints) {
        return (numPoints + 1) * 2;
    }

    static GeneratedData createPuck(Geometry.Cylinder puck, int numPoints) {
        int size = sizeOfCircleInVertices(numPoints)
                + sizeOfOpenCylinderInVertex(numPoints);
    ...
    static GeneratedData createMallet(Geometry.Point center, float radius, float height, int numPoints) {
        int size = sizeOfCircleInVertices(numPoints) * 2 + sizeOfOpenCylinderInVertex(numPoints) * 2;
        ObjectBuilder builder = new ObjectBuilder(size);
    ...
    private void appendCircle(Geometry.Circle circle, int numPoints) {
        ...
        for (int i = 0; i <= numPoints; i++) {
            ...
            Log.e(TAG, "appendCircle: "+offset );
        }
        drawList.add(new DrawCommend() {
            @Override
            public void draw() {
    ...
    private void appendOpenCylinder(Geometry.Cylinder cylinder, int numPoints) {
        final int startVertex = offset / FLOATS_PER_VERTEX;
        final int numVertices = sizeOfOpenCylinderInVertex(numPoints);
